...
/* Product Name:Micron M25P80 */
#define SPI_MEMORY_NAME					"M25P80"
...
extern void memory_write_byte(MQX_FILE_PTR spifd,
							  uint32_t addr, unsigned char data);
extern uint8_t memory_read_byte(MQX_FILE_PTR spifd, uint32_t addr);
extern uint32_t memory_write_data(MQX_FILE_PTR spifd,
								  uint32_t addr, uint32_t size,
								  const unsigned char *data);
extern uint32_t memory_read_data(MQX_FILE_PTR spifd,

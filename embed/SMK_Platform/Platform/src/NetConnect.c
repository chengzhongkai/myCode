...
#if 1
				uint32_t reg[32];
				if ((!(ipcfg_phy_registers(BSP_DEFAULT_ENET_DEVICE, 32, reg))) ||
				    ((reg[31] & 0x18) != 0x18) )
				{
#if _DEBUG
	 				puts("Net_Connect_task: Retry auto negotiation.\n");
#endif
...
#if _DEBUG
	 				puts("Net_Connect_task: Auto negotiation end.\n");
#endif
...
//				    ((info->isUseDhcp == true) && (_time_diff_seconds(&currenttime, &staticconnectchecktime, &overflow) >= NET_CONNECT_WATCH_PERIOD)) )
			...
			case NetConnect_Disconnect:
				...
				{
					error = ipcfg_release_device(BSP_DEFAULT_ENET_DEVICE);
					if (error != IPCFG_OK)
					{
#if _DEBUG
						fprintf(stderr, "Net_Connect_task: Fatal Error 0x%X: Network device release failed.\n", error);
#endif
						dwConnectStatus = NetConnect_Error;
					}
					else
					{
#if _DEBUG
	 					puts("Net_Connect_task: Ethernet Driver released.\n");
#endif
						dwConnectStatus = NetConnect_Init;
					}
				}

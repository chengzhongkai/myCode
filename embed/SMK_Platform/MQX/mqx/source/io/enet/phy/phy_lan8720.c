...
static bool phy_lan8720_get_duplex(ENET_CONTEXT_STRUCT_PTR enet_ptr);
...
{ 
   ...
   if (enet_ptr->PARAM_PTR->OPTIONS & ENET_OPTION_PHY_LOOPBACK) {
	  ...
	  switch (enet_ptr->PARAM_PTR->MODE)
	  {
	   	  case Half_Duplex_10M:

	   		  phy_status &= ~(LAN8720_REG_BMCR_AUTO_NEG_ENABLE | LAN8720_REG_BMCR_FORCE_SPEED_100 | LAN8720_REG_BMCR_FORCE_FULL_DUP);
	   		  break;

	   	  case Full_Duplex_10M:

		   	  phy_status &= ~(LAN8720_REG_BMCR_AUTO_NEG_ENABLE | LAN8720_REG_BMCR_FORCE_SPEED_100);
		   	  phy_status |= LAN8720_REG_BMCR_FORCE_FULL_DUP;
		   	  break;

	   	  case Half_Duplex_100M:

			  phy_status |= LAN8720_REG_BMCR_FORCE_SPEED_100;
		   	  phy_status &= ~(LAN8720_REG_BMCR_AUTO_NEG_ENABLE | LAN8720_REG_BMCR_FORCE_FULL_DUP);
	   		  break;

	   	  case Full_Duplex_100M:

	   		phy_status &= ~(LAN8720_REG_BMCR_AUTO_NEG_ENABLE);
	   		phy_status |= (LAN8720_REG_BMCR_FORCE_SPEED_100 | LAN8720_REG_BMCR_FORCE_FULL_DUP);
	   		break;

	   	  case Auto_Negotiate:

	   		phy_status |= (LAN8720_REG_BMCR_AUTO_NEG_ENABLE | LAN8720_REG_BMCR_RESTART_AUTONEG);
	   		break;
	 ...
	 if (!((*enet_ptr->PARAM_PTR->ENET_IF->MAC_IF->PHY_WRITE)(enet_ptr, LAN8720_REG_BMCR, phy_status, MII_TIMEOUT))) {
	    return FALSE;
	 }
...
/*FUNCTION*-------------------------------------------------------------
*
*  Function Name  : phy_lan8720_get_duplex
*  Returned Value : TRUE  - full duplex
*  					FALSE - half duplax
*  Comments       :
*    Determine if duplex setting is full
*
*END*-----------------------------------------------------------------*/

static bool phy_lan8720_get_duplex
   (
       ENET_CONTEXT_STRUCT_PTR     enet_ptr
   )
{
   ...
   if ((*enet_ptr->PARAM_PTR->ENET_IF->MAC_IF->PHY_READ)(enet_ptr, LAN8720_REG_PHY_SCSR, &data, MII_TIMEOUT)) {
      if (data & LAN8720_REG_PHY_SCSR_DUPLEX_MASK) {
         res = TRUE;
      }
   }
   return res;

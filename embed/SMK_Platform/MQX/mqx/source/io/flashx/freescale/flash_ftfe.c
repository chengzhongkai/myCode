...
static void ftfe_ram_function(volatile uint8_t *); 
...
*/
	...
	if (command_array[0] == FTFE_READ_RESOURCE)
	{
        command_array[4]  = ftfe_ptr->FCCOB4;
		command_array[5]  = ftfe_ptr->FCCOB5;
		command_array[6]  = ftfe_ptr->FCCOB6;
		command_array[7]  = ftfe_ptr->FCCOB7;
		command_array[8]  = ftfe_ptr->FCCOB8;
		command_array[9]  = ftfe_ptr->FCCOB9;
		command_array[10] = ftfe_ptr->FCCOBA;
		command_array[11] = ftfe_ptr->FCCOBB;
	}
...
)
{
    ...
    */
    ...
    flexnvm_set_flexram(dev_ptr, (unsigned char)FLEXNVM_FLEXRAM_EE);
#endif
...
)
{
    ...
    /* preparing passing parameter to erase a flash block */
    command_array[0] = FTFE_READ_RESOURCE;
    command_array[1] = (uint8_t)((param_ptr->ADDR >> 16) & 0xFF);
    command_array[2] = (uint8_t)((param_ptr->ADDR >> 8) & 0xFF);
    command_array[3] = (uint8_t)(param_ptr->ADDR & 0xF8);
 	command_array[4] = (uint8_t)(param_ptr->RSRC_CODE);

    /* call flash command sequence function to execute the command */
    if (FTFE_OK != ftfe_flash_command_sequence(dev_spec_ptr, command_array, 12, NULL, 0)) {
        return FALSE;
    }

    param_ptr->RD_DATA =((uint32_t)command_array[10] << 8) + (uint32_t)command_array[11];
...
)
{
    ...
    /* preparing passing parameter to erase a flash block */
    command_array[0] = FTFE_PROGRAM_PARTITION;
    command_array[1] = 0;
    command_array[2] = 0;
    command_array[3] = 0;
    ...
    /* call flash command sequence function to execute the command */
    if (FTFE_OK != ftfe_flash_command_sequence(dev_spec_ptr, command_array, 6, NULL, 0)) {
        return FALSE;
    }
...
)
{
    ...
    command_array[0] = FTFE_SET_EERAM;
    command_array[1] = ctrl_code;

    /* call flash command sequence function to execute the command */
    if (FTFE_OK != ftfe_flash_command_sequence (dev_spec_ptr, command_array, 2, NULL, 0)) {
        return FALSE;
    }
...
)
{
    ...
    switch (cmd) {
        ...
        case FLEXNVM_IOCTL_WAIT_EERDY: {
                FTFE_FLASH_INTERNAL_STRUCT_PTR dev_spec_ptr = dev_ptr->DEVICE_SPECIFIC_DATA;
                FTFE_MemMapPtr ftfe_ptr = (FTFE_MemMapPtr)dev_spec_ptr->ftfe_ptr;

                while (!(ftfe_ptr->FCNFG & FTFE_FCNFG_EEERDY_MASK)){}
        ...
        case FLEXNVM_IOCTL_GET_EERDY: {
                ...
                if (NULL != param_ptr)
                    *(uint32_t*)param_ptr = (ftfe_ptr->FCNFG & FTFE_FCNFG_EEERDY_MASK);

...
{ /* Body */
   ...
   ucb->PCOUNT       = 0;
   ucb->IPADDR       = ipaddr;
   ucb->KEEP_IPADDR  = (bool) (ipaddr != INADDR_ANY);
...
{ /* Body */
   ...
   ucb->KEEP_IPADDR  = (bool) (!IN6_ARE_ADDR_EQUAL(&(ipv6address), &in6addr_any));
   ucb->SOCKET       = parms->udpword;
   ucb->SERVICE      = parms->udpservice;
...
{ /* Body */
   UDP_CFG_STRUCT_PTR   UDP_cfg_ptr;
   UCB_STRUCT_PTR       ucb = parms->ucb, search_ptr, * tmp_ptr;
   UCB_STRUCT_PTR            *ins_ptr, * del_ptr;
   uint16_t              port;
   uint32_t              error = parms->udpword;
...
{ /* Body */
   ...
   */
   ...
   if(pcb_ptr)
   {
      ...
      */
      IN6_ADDR_COPY((in6_addr *)(((IP6_HEADER_PTR)(pcb_ptr->NETWORK_LAYER))->SOURCE),&(parms->ipv6address));
...
{ 
   ...
   mqx_htons(packet->SRC_PORT, ucb->PORT);
   mqx_htons(packet->DEST_PORT, destport);
   mqx_htons(packet->LENGTH, RTCSPCB_SIZE(pcb_ptr));
   ...
   {
        return IP6_send(pcb_ptr, IPPROTO_UDP, NULL, &destaddr, pcb_ptr->IFSRC);
   }
   else
   {
        return IP6_send(pcb_ptr, IPPROTO_UDP, &srcaddr, &destaddr, pcb_ptr->IFSRC);
   }
} /* Endbody */
...
{ /* Body */
   ...
   RTCSPCB_DATA_TRANSPORT(pcb_ptr) = RTCSPCB_DATA(pcb_ptr);
   mqx_htons(packet->SRC_PORT, ucb->PORT);
   mqx_htons(packet->DEST_PORT, dest_port);
   mqx_htons(packet->LENGTH, RTCSPCB_SIZE(pcb_ptr));
   mqx_htons(packet->CHECKSUM, 0);
...
{ /* Body */
   UDP_CFG_STRUCT_PTR   UDP_cfg_ptr;
   UDP_HEADER_PTR       packet;
   uint16_t              port, src_port, len, sum;
   ...
   */
   if(!ucb_ptr && UDP_cfg_ptr->BOUND_UCB_HEAD)
   {
      ...
      /* Check the message was multicast, and if the ucb is a member */
        ...
        {
            if(!ip6_multicast_find_socket_entry(((SOCKET_STRUCT_PTR)ucb_ptr->SOCKET), pcb_ptr->IFSRC, &ipv6dest )) 
                ucb_ptr = NULL;
        }
    }
...
{ /* Body */
   ...
   */
   ...
   if (req_ptr)
   {
      ...
      IN6_ADDR_COPY((in6_addr *)(((IP6_HEADER_PTR)(pcb_ptr->NETWORK_LAYER))->SOURCE),&(req_ptr->ipv6address));
...
{ /* Body */
   ...
   */
   ...
   #endif  
   #if RTCSCFG_ENABLE_IP6
      || IN6_ARE_ADDR_EQUAL(&ucb->REMOTE_HOST6, &in6addr_any)
   #endif
      )
   {
      uint16_t family = ucb->AF;
